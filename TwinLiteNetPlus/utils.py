...
def val(val_loader=None, model=None, half=False, args=None):
    ...
    for i, (_, input, target) in pbar:
        ...
        _, da_pred = torch.max(out_da, 1)
        da_pred = da_pred[:, 12:-12]
        _, da_gt = torch.max(target_da, 1)

        DA.addBatch(da_pred.cpu(), da_gt.cpu())

        # -------- Lane Line --------
        out_ll = output[1]
        target_ll = target[1]

        _, ll_pred = torch.max(out_ll, 1)
        ll_pred = ll_pred[:, 12:-12]
        _, ll_gt = torch.max(target_ll, 1)

        LL.addBatch(ll_pred.cpu(), ll_gt.cpu())

    # Compute metrics from full confusion matrix
    da_metrics = DA.getBinaryMetrics()
    ll_metrics = LL.getBinaryMetrics()

    return da_metrics, ll_metrics
