...
def validation(args):
    ...
    # Perform validation
    da_metrics, ll_metrics = val(valLoader, model, args.half, args=args)
    
    # Print results
    print(
        f"Drivable Area | "
        f"PixelAcc: {da_metrics['pixel_acc']:.3f}, "
        f"IoU: {da_metrics['iou']:.3f}, "
        f"mIoU: {da_metrics['miou']:.3f}, "
        f"F1: {da_metrics['f1']:.3f}"
    )

    print(
        f"Lane Line | "
        f"PixelAcc: {ll_metrics['pixel_acc']:.3f}, "
        f"IoU: {ll_metrics['iou']:.3f}, "
        f"mIoU: {ll_metrics['miou']:.3f}, "
        f"F1: {ll_metrics['f1']:.3f}"
    )
